...
让 AI 调用提供的工具。
但是如果要实时展示工具调用信息，就很复杂了。

### 方案2 Agent 模式

智能体（Agent）以大语言模型为核心，集成 记忆、知识库、工具等能力为一体。

但整体架构更复杂，需要自己设计 Agent 流程；而且多次调用 AI，输出不可控，成本也更高。
...
维度 Dimension：用来描述和分类数据的标签属性，如 用户 ID，应用 ID，模型名称等。

指标 Metric：用来量化的数值数据，如请求次数、响应时间、Token 消耗量等。

**监控的数据分类**

- 系统指标：CPU、内存、IO占用等
- 应用指标：接口响应时间、QPS、错误率、JVM 状态等
- 业务指标
- 调用链

**百分位数**

- P50：中位数，表示 50% 的请求响应时间都在这个值之下
- P75
- P90
- P99

其中，P99 指标对于发现系统中的异常情况特别有用，能反映出偶发的
长尾延迟的问题。

注意，和优先级的 P0、P1、P2 不同。
...
阿里云提供的应用实时监控服务。采用探针技术，能够在不修改应用代码的情况下，
自动获取应用性能数据。
...
需要手动埋点，在业务层收集最原始、细粒度的数据。

### Prometheus

开源的监控系统，专门为时序数据的收集、存储和查询而设计。
将所有监控数据以 **时间序列** 的形式存储。

关键组件：

- Prometheus Server：系统核心，负责数据收集、存储和查询。定期 **拉取** 指标数据
- Exporter：将第三方系统（如数据库）指标转换为 Prometheus 格式
- AlertManager：处理告警规则和通知分发
- 客户端库：提供各种编程语言的 SDK

采用**拉取模式**来收集指标数据，而不是项目主动推送。
...
软件架构风格，将应用程序分为一组小型、独立的服务。每个服务围绕特定的业务功能构建。
通过明确定义的 API 进行通信。

- 每个微服务可以独立开发、测试、部署和扩展；
- 按业务功能划分，高内聚、低耦合
- 技术选型灵活，不同微服务可以根据自身特性选择最合适的技术栈
- 弹性扩展，可以根据业务负载情况，针对性地扩展高压力服务
- 故障隔离
...
DDD 领域驱动设计和微服务是两码事，DDD 是一种软件设计方法论，而微服务是一种架构风格。
DDD 可以指导微服务的拆分，但不等同于微服务本身。

## 架构设计

- 设计上：怎么划分服务？
- 实现上：用什么技术？用什么技术栈支持微服务架构？
- 治理上：如何管理服务？服务注册发现、配置管理、监控？

### 服务拆分

遵循以下原则：

1. 业务领域驱动：按照业务功能拆分，业务逻辑相对独立。
2. 数据一致性：相关性强的数据保持在同一个服务中，避免跨服务查询。
3. 团队组织：考虑团队规模和技能分工，确保每个服务有专门团队负责。
4. 性能考虑：将重计算模块独立出来，便于单独优化和扩展。
5. 扩展性需求：根据不同模块的扩展需求进行拆分。

**拆分微服务一定要灵活，不能拆的不要硬拆**。

### 技术选型

需要解决服务拆分、服务间通信、服务发现、统一网关等核心问题。
本项目使用 Spring Cloud Alibaba、Nacos、Dubbo、Higress 网关。
